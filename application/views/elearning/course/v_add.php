...
<script type="text/javascript">
    ...
    $(document.body).on("click", "#btn-respon", function(event) {
        update_respon();
    });
...
<script type="text/javascript">
    ...
    function update_respon() {
        $.ajax({
            url: module + "_do/ajax/type/action/source/respon",
            dataType: "json",
            type: "POST",
            data: { 
                id: $("#jawab_id").val(), soal: $("#soal_id").val(),
                valid: $("input[name='valid']:checked").val(), nilai: $("#nilai").val(), catatan: $("#catatan").val()
            },
            success: function (rs) {
                if (rs.status) {
                    list_soal($("#btn-ragu").attr("itemid"));
    ...
    function tanya_pertama(){
        ...
        bootbox.confirm({ title: title, message: msg, 
            buttons: {
                ...
                confirm: {
                    label: "<i class='ace-icon fa fa-check bigger-110'></i> Iya, Akhiri Sesi Ini", className: "btn btn-danger"
                }
            },
